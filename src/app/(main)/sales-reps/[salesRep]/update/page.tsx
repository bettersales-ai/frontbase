...
interface UpdateSalesRepProps {
  params: Promise<{ salesRep: string }>;
}

type Res = Awaited<ReturnType<typeof getSalesRep>>;

const UpdateSalesRep = ({ params }: UpdateSalesRepProps) => {
  const router = useRouter();
  const { salesRep } = React.use(params);

  const [step, setStep] = React.useState(1);
  const [salesRepData, setSalesRepData] = React.useState<Res | null>(null);

  React.useEffect(() => {
    (async () => {
      const data = await getSalesRep(salesRep);
  ...
  const formik = useFormik({
    initialValues: {
      sop: salesRepData?.sop || "",
      name: salesRepData?.name || "",
      initial_message: salesRepData?.initial_message || "",
      phoneNumber: salesRepData?.whatappCredentials?.phoneNumber || "",
      accessToken: salesRepData?.whatappCredentials.accessToken || "",
      phoneNumberId: salesRepData?.whatappCredentials.phoneNumberId || "",
      ideal_customer_profile: salesRepData?.ideal_customer_profile || "",
    },
    validationSchema,
  ...
  return (
    <div className="flex flex-col items-center gap-9 pt-16 pb-8 w-full h-full">
      <div className="text-center space-y-1">
        <h1 className="mb-2 text-5xl tracking-tight font-extrabold text-gray-900">Create an SalesRep</h1>
